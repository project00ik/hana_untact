...
hanaProdUI.dialSelect = function(obj, param, cfn){
    ...
    */
    ...
    function createYY(o, unitText){
        ...
        o.scrollWrapper[o.groupIdx].scrollItems.find('.list-section').eq(0).scrollTop(0).find('.list-wrap__item').eq(0).addClass('active-item').css('background','skybule').attr({
            "role": "button",
            "title": "선택됨",
            "aria-pressed": true
        }).siblings().attr({
    ...
    function event($el, o){
        ...
        $el.find('.list-section').each(function(){
            ...
            var orgTargetIdx = 0;
            //스크롤시 이벤트
            scrollContainer.off('scroll').on('scroll focus',function(){
                var me = $(this);
